      ...
      <main ref={containerRef} className="relative w-full max-w-7xl mx-auto p-4 md:p-12 pb-32 space-y-24 md:space-y-32">
        ...
        <section className="relative py-10">
          ...
          <div className="md:hidden mt-8 relative">
            <div className="overflow-x-auto scroll-smooth snap-x snap-mandatory evidence-carousel">
              <div className="flex gap-4 px-4 pb-4" style={{ width: 'max-content' }}>
                {EVIDENCE_GALLERY.map((item, index) => (
                  <div 
                    key={item.id} 
                    className="relative group perspective-1000 flex-shrink-0 snap-start"
                    style={{ 
                ...
                {/* Preview peek for next item - always visible (shows first image as preview) */}
                {EVIDENCE_GALLERY.length > 0 && (
                  <div 
                    className="flex-shrink-0 opacity-40"
                    style={{ width: 'calc(25vw - 1rem)' }}
                  >
                    <div className="w-full h-full pointer-events-none">
                      <Polaroid 
                        src={EVIDENCE_GALLERY[0].url} 
                        caption=""
                        rotation={2}
                        className="w-full"
                      />
            ...
            <div className="absolute right-0 top-0 bottom-4 w-20 bg-gradient-to-l from-[#3f2e21] to-transparent pointer-events-none z-10"></div>
          </div>

          {/* Desktop: Grid Layout */}
          <div className="hidden md:grid md:grid-cols-4 gap-4 md:gap-8 mt-8">
            {EVIDENCE_GALLERY.map((item, index) => (
