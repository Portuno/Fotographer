...
export const Polaroid: React.FC<{ 
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden filter sepia-[0.3] contrast-125 group min-h-[200px] bg-gray-200">
        {imageError ? (
          <div className="w-full h-full flex items-center justify-center bg-gray-300">
            <div className="text-center p-4">
              <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mx-auto text-gray-500 mb-2">
                <path d="M14.5 4h-5L7 7H4a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-3l-2.5-3z"/>
                <circle cx="12" cy="13" r="3"/>
              </svg>
              <p className="text-xs text-gray-600 font-typewriter">Imagen no disponible</p>
            </div>
          </div>
        ) : (
          <>
            {imageLoading && (
              <div className="absolute inset-0 flex items-center justify-center bg-gray-200">
                <div className="animate-pulse text-gray-400 text-xs font-typewriter">Cargando...</div>
              </div>
            )}
            <img 
              src={src} 
              alt={caption || "Evidence"} 
              className={`w-full h-auto object-cover pointer-events-none ${imageLoading ? 'opacity-0' : 'opacity-100'} transition-opacity duration-300`}
